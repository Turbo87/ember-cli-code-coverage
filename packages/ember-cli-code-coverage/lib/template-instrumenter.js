...
module.exports = class IstanbulInstrumenter {
  constructor(options) {
    ...
    this.coverageData = {
      path: this.options.meta.moduleName,
      s: {},
      b: {},
      f: {},
      fnMap: {},
  ...
  insertHelper(container, node, hash) {
    ...
    hash.pairs.push(b.pair('path', b.string(this.options.meta.moduleName)));

    let helper = b.mustache(b.path('ember-cli-code-coverage-increment'), null, hash);
  ...
  insertStatementHelper(node) {
    ...
    let hash = b.hash([b.pair('statement', b.string(this._currentStatement))]);
  ...
  handleBlock(node) {
    ...
    if (this.currentContainer()._ignoreCoverage) {
      return;
    }
  ...
  handleStatement(node) {
    ...
    if (!node.loc || node.loc.start.line == null) {
      return;
    }
      ...
      Program: {
        ...
        exit: node => {
          ...
          if (node === this._topLevelProgram) {
            let helper = b.mustache(b.path('ember-cli-code-coverage-register'), [
